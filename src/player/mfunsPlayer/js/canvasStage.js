...
export class canvasStage {
  constructor(tem, advanceDanmakuStage, options = {}) {
    ...
    this.chosenBarrage = [];
    tem.control_mask.addEventListener(
      "click",
      function (e) {
        let p = that.getp(e);
    ...
    this.video.addEventListener("canplay", function () {
      ...
      tem.advance.style.height =
        tem.video.clientHeight - tem.mask.clientHeight + "px";
      tem.ade_mask.style.height =
        tem.video.clientHeight - tem.mask.clientHeight + "px";
  ...
  renderTop(p) {
    ...
    this.topbarrage.items.forEach((barrage, index) => {
      if (
        !barrage.element.topend &&
        videoTime >= barrage.element.time &&
        videoTime <= barrage.element.time + 4
      ) {

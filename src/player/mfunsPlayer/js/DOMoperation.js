...
export function operate(canvasStage, advanceDanmakuStage,tem ,BarrageData,callback) {
   ...
   tem.video.addEventListener('progress',function(){
	...
	//选择弹幕颜色
	const colorArr=['#FE0302','#FFFF00','#00CD00','#00FF00','#4e6ef2',
	'#89D5FF','#7b7ff7','#757575','#ffffff','#fb7299']
	...
	document.onkeydown = function(event) {
		...
		if (e && e.keyCode == 37 && tem.video.currentTime >= 0) { // left键
			tem.video.currentTime -= 5
			tem.load.value = tem.video.currentTime
			isMinus = true
			minus5()
		}
		if (e && e.keyCode == 39  && tem.video.currentTime >= 0) { // right键
			tem.video.currentTime += 5
			tem.load.value = tem.video.currentTime
			isAdd = true
			add5()
		}
        if (e && e.keyCode == 38  && tem.video.currentTime >= 0) { // top键
		   tem.voice.style.display = 'block'
		    voiceDebounceFunc()
        	handleIncrease()
        }
		if (e && e.keyCode == 40  && tem.video.currentTime >= 0) { // bottom键
		    tem.voice.style.display = 'block'
		    voiceDebounceFunc()
		    handleDecrease()
		}
		if (e && e.keyCode == 32 && !isInput) { // space键
			handleClick()
		}
